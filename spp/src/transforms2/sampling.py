...
class SaveNodeIndex(Transform):
    ...
    def _process(self, data):
        ...
        # data.point_id = torch.arange(data.pos.shape[0], device=data.pos.device)
        return data
...
def optimized_point_processing(data, data_in):
    total_points = data.pos.shape[0]
    device = data.pos.device
    start_time = time.time()
    
    print(f"Starting optimized processing for {total_points} points...")
    ...
    # 批量预处理所有sub values
    with torch.cuda.amp.autocast():  # 使用混合精度来提升性能
        # 批量收集所有indices
        processed_indices = []
        
        for i in tqdm(range(total_points), desc="Processing indices"):
            values = data['sub'][i].values
            
            if isinstance(values, list) and len(values) == 1:
                tensor_value = values[0]
                if isinstance(tensor_value, torch.Tensor):
                    if tensor_value.numel() == 1:
                        indices = [tensor_value.item()]
                    else:
                        indices = tensor_value.tolist()
                else:
                    indices = [int(tensor_value)]
                ...
                indices = [int(idx) for idx in values]
                
            processed_indices.append(indices)
            voxel_to_points[i] = indices
    
    # 批量处理原始点坐标
    print("Computing point coordinates and means...")
    mean_positions = torch.zeros_like(data.pos)
    ...
    with torch.cuda.amp.autocast():
        for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
            ...
            batch_indices = processed_indices[start_idx:end_idx]
            
            # 批量获取原始点坐标
            batch_original_points = [data_in.pos[indices] for indices in batch_indices]
            batch_mean_pos = torch.stack([points.mean(dim=0) for points in batch_original_points])
            
            mean_positions[start_idx:end_idx] = batch_mean_pos
            all_original_points.extend(batch_original_points)
            
            # 输出前5个点的验证信息（只在第一个批次）
            if batch_idx == 0:
                for i in range(min(5, len(batch_indices))):
                    print(f"\nVoxel point {i}:")
                    print(f"  Voxel coordinates: {data.pos[i]}")
                    print(f"  Original points coordinates:")
                    for j, idx in enumerate(batch_indices[i]):
                        print(f"    Point {j}: {data_in.pos[idx]}")
                    print(f"  Mean of original points: {batch_mean_pos[i]}")
                    print(f"  Difference: {torch.abs(batch_mean_pos[i] - data.pos[i])}")
                    print(f"  Is the difference small enough? {torch.allclose(batch_mean_pos[i], data.pos[i])}")
                    print("--------------------")
    
    total_time = time.time() - start_time
    print(f"\nProcessing complete!")
    print(f"Total processing time: {total_time:.2f} seconds")
    print(f"Average time per point: {total_time/total_points:.4f} seconds")
    
    return voxel_to_points, all_original_points, mean_positions
...
class GridSampling3D(Transform):
    ...
    def _process(self, data_in):
        ...
        try:
            voxel_to_points, all_original_points, mean_positions = optimized_point_processing(data, data_in)
        ...
        # 在"2"文件夹下生成完整的文件路径
        scene_folder=data.scene_id
        output_file = os.path.join(save_dir, f"{scene_folder}_same.pt")
        output_file2 = os.path.join(save_dir, f"{scene_folder}_same_full2.pt")

        output_data = {
            "voxel_to_points": voxel_to_points,  # 体素到原始点的映射关系
        }

        # 将原始点集合并成一个张量
        all_original_points = torch.cat(all_original_points, dim=0)
        output_data2 = {
            "voxel_to_points": voxel_to_points,  # 体素到原始点的映射关系
            "ori_pos": all_original_points  # 所有原始点的坐标
        }

        torch.save(output_data, output_file)
        torch.save(output_data2, output_file2)

        print(f"体素到原始点的映射关系已保存到 {output_file}")
        print(f"体素到原始点的映射关系和坐标已保存到 {output_file2}")
        ...
        #     original_point_indices = data['sub'][i].values

        #     # print(f"Type of original_point_indices: {type(original_point_indices)}")
        #     # print(f"Length of original_point_indices: {len(original_point_indices)}")

        #     if isinstance(original_point_indices, list) and len(original_point_indices) == 1:
        #         # 如果是列表且长度为1，需要进一步处理其中的张量
        #         tensor_value = original_point_indices[0]
        #         # print(f"Processing tensor value: {tensor_value}")
                
        #         if isinstance(tensor_value, torch.Tensor):
        #             if tensor_value.numel() == 1:
        #                 # 单元素张量
        #                 original_point_indices = [tensor_value.item()]
        #             else:
        #                 # 多元素张量
        #                 original_point_indices = tensor_value.cpu().tolist()
        #         else:
        #             original_point_indices = [int(tensor_value)]
        #     elif isinstance(original_point_indices, torch.Tensor):
        #         if original_point_indices.numel() == 1:
        #             original_point_indices = [original_point_indices.item()]
        #         else:
        #             original_point_indices = original_point_indices.cpu().tolist()
        #     else:
        #         # 处理其他情况
        #         original_point_indices = [int(idx) for idx in original_point_indices]

        #     voxel_to_points[i] = original_point_indices

        #     original_points = data_in.pos[original_point_indices]
        #     all_original_points.append(original_points)  # 将原始点坐标加入列表
        #     # 计算这些原始点的平均坐标
        #     mean_pos = original_points.mean(dim=0)
            
        #     # 输出验证信息
        #     if i < 5:
        #         print(f"Voxel point {i}:")
        #         print(f"  Voxel coordinates: {data.pos[i]}")
        #         print(f"  Original points coordinates:")
        #         for j, idx in enumerate(original_point_indices):
        #             print(f"    Point {j}: {data_in.pos[idx]}")
        #         print(f"  Mean of original points: {mean_pos}")
        #         print(f"  Difference: {torch.abs(mean_pos - data.pos[i])}")
        #         print(f"  Is the difference small enough? {torch.allclose(mean_pos, data.pos[i])}")
        #         print("--------------------")

        # # 保存体素到原始点的映射关系和其他数据到 .pt 文件
        # output_file = "0011_00_same2.pt"
        # output_data = {
        #     "voxel_to_points": voxel_to_points,  # 体素到原始点的映射关系
        # }
        ...
        # # 保存体素到原始点的映射关系和其他数据到 .pt 文件
        # output_file = "0011_00_same_full2.pt"
        # output_data = {
        #     "voxel_to_points": voxel_to_points,  # 体素到原始点的映射关系
        #     "ori_pos": all_original_points  # 所有原始点的坐标
        # }
