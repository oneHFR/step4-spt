...
# [2025-02-11 18:20:27,693][src.datasets.base][INFO] - Dataset hash: "4c86be492e4e9b7aa26f266479b89496"
# [2025-02-11 18:20:34,409][src.datasets.base][INFO] - Preprocessed data can be found at: "/root/autodl-tmp/spp/data/scannet/processed/<stage>/4c86be492e4e9b7aa26f266479b89496"
    ...
    def process(self) -> None:
        ...
        # # 直接修改 self.processed_paths 为新的路径列表
        # p = '/root/autodl-tmp/spp/data/scannet/processed/val/4c86be492e4e9b7aa26f266479b89496/scans/scene0011_00.h5'
    ...
    def _process_single_cloud(self, cloud_path: str) -> None:
        ...
        validation_dir = '/root/autodl-tmp/spp/data/scannet/processed_o3d/instance_gt/validation'
        scans_dir = '/root/autodl-tmp/spp/data/scannet/raw/scans'
        validation_scenes = set()
        for filename in os.listdir(validation_dir):
            # 提取场景名称（去掉可能的文件扩展名）
            scene_name = os.path.splitext(filename)[0]
            validation_scenes.add(scene_name)
        
        print(f"Found {len(validation_scenes)} validation scenes")
        
        start_id = '0700'  # 示例 start_id
        for scene_folder in tqdm(os.listdir(scans_dir), desc="Processing validation scenes"):
            # 检查是否是validation场景
            if scene_folder not in validation_scenes:
                continue


            # 提取数字部分并转换为整数，假设命名规则为 sceneXXXX_XX
            scene_number_str = scene_folder[5:].split('_')[0]  # 去掉前面的 'scene' 和下划线后提取数字部分
            scene_id = int(scene_number_str)  # 转换为整数

            # 控制起始位置，只有 scene_id 大于 start_id 才进行处理
            if scene_id <= int(start_id):
                continue  # 跳过处理

            scene_path = os.path.join(scans_dir, scene_folder)
            if not os.path.isdir(scene_path):
                continue
                
            print(f"\nProcessing validation scene: {scene_folder}")

            
            try:
                data = self.sanitized_read_single_raw_cloud(scene_path)
                ...
                if self.pre_transform is not None:
                    ...
                    print(f"Applying pre_transform to {scene_folder}")
                    data.scene_id = scene_folder
                    nag = self.pre_transform(data)
                    process_time = time.time() - start_time
                    print(f"Completed processing {scene_folder} in {process_time:.2f} seconds")
                ...
                print(f"Error processing scene {scene_folder}: {str(e)}")
                continue
