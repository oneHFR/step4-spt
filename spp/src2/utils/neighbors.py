import torch
import src2
from src2.dependencies.FRNN import frnn
from torch_scatter import scatter
from torch_geometric.utils import coalesce
from src2.utils.scatter import scatter_nearest_neighbor
...
def knn_1(
    ...
    WHR
    """
    # Save partition result as superpoint.pt
    partition_labels = neighbors.argmax(dim=1)  # 假设分簇标签为邻居最大值的索引
    torch.save(partition_labels, 'superpoint.pt')  # 保存为superpoint.pt文件
...
def knn_1_graph(
    ...
    # Save partition result as superpoint.pt
    partition_labels = neighbors.argmax(dim=1)  # 假设分簇标签为邻居最大值的索引
    torch.save(partition_labels, 'superpoint_graph.pt')  # 保存为superpoint.pt文件
